...
def test_action_is_recomputed_from_current_observation():
    policy = VanillaRLPolicy.__new__(VanillaRLPolicy)
    policy.model = _PredictRecorder()
    obs_t = np.array([1.0, 2.0, 3.0], dtype=np.float32)
    obs_next = np.array([4.0, 5.0, 6.0], dtype=np.float32)
    action_t = policy.act(obs_t, deterministic=True)
    action_next = policy.act(obs_next, deterministic=True)
