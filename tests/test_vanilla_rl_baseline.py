...
def test_plot_run_old_and_new_schema(tmp_path):
    ...
    subprocess.run(
        [sys.executable, "-m", "rl_baselines.plot_run", "--run", str(old)],
        cwd=ROOT,
        check=True,
    ...
    live = {
        ...
        "rolling_episode_length": 200,
        "rolling_max_object_height": 0.83,
        "rolling_object_lift_delta": 0.001,
        "rolling_grasp_rate": 0.1,
        "rolling_grasp_duration": 1,
        "action_norm": 1,
        "action_saturation_fraction": 0,
        "rolling_return_per_step": 0.005,
    }
    ev = {
        "step": 1000,
        "success_rate": 0,
        ...
        "mean_return_per_step": 0.005,
        "episodes": 20,
        "grasp_rate": 0,
        "mean_max_object_height": 0.83,
    }
    for name, row in [("live_metrics.csv", live), ("learning_curve.csv", ev)]:
        with (tmp_path / name).open("w", newline="") as f:
            ...
            w.writerow(row)
    subprocess.run(
        [sys.executable, "-m", "rl_baselines.plot_run", "--run", str(tmp_path)],
        cwd=ROOT,
        check=True,
        stdout=subprocess.DEVNULL,
    )
    assert (tmp_path / "figures/grasp_rate.png").exists() and (
        tmp_path / "figures/return_per_step.png"
    ).exists()
...
def test_resume_audit_is_honest():
    x = json.loads(
        (
            ROOT
            / "results/vanilla_rl_baseline_v1/sac_20k_sanity_tqdm/posthoc_diagnostic_audit.json"
        ).read_text()
    )["resume_audit"]
...
def test_checkpoint_selection_success_first():
    ...
    ranking_fields = (
        'm["success_rate"]',
        'm["lift_rate"]',
        'm.get("median_object_lift_delta")',
        'm["grasp_rate"]',
        'm["mean_return"]',
    )
    assert all(field in src for field in ranking_fields)
    assert src.index(ranking_fields[0]) < src.index(ranking_fields[-1])


def test_return_per_step_and_action_diagnostics():
    from rl_baselines.envs import make_nominal_env

    e = make_nominal_env(seed=7, horizon=2)
    o, _ = e.reset(seed=7)
    assert o.shape == (42,) and np.isfinite(o).all()
    a = np.zeros(7, np.float32)
    o, r, t, tr, info = e.step(a)
    o, r2, t, tr, info = e.step(a)
    d = info["episode_diagnostics"]
    e.close()
    assert tr and np.isclose(d["return_per_step"], d["episode_return"] / d["episode_length"])
    assert (
        len(d["action_mean_per_dimension"]) == 7 and len(d["action_saturation_per_dimension"]) == 7
    )
    assert (
        isinstance(d["ever_grasped"], bool) and d["max_object_height"] >= d["initial_object_height"]
    )
...
def test_original_c_dataset_and_classifier_unchanged():
    ...
    assert (
        sha("/home/robotics/external_workspace/data/C/demos/lift_ph_demo.hdf5")
        == "2394eb28fd6fe0a5e733a262689b7bc4141ba1940bd1397652ae6a26ffd105c5"
    )
    assert (
        sha("/home/robotics/external_workspace/train/runs/final_factor_graph_plus_extra/best.pt")
        == "11ea80908b505e7c15e4e0f43b45bc3611cca115a9fdf4a9fce029628ec47b16"
    )
