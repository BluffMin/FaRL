#!/usr/bin/env python3
"""Build the canonical vanilla SAC provenance documentation from saved artifacts."""
...
DOC = ROOT / "docs"
OUT = DOC / "vanilla_sac_baseline_recipe"
OUT.mkdir(parents=True, exist_ok=True)
FIX = BASE / "sac_seed0_300k_fixed_horizon_from_100k"
PARENT = BASE / "sac_seed0_100k_fixed_horizon"
CTRL = BASE / "sac_seed0_300k_from_last"
REWARD = BASE / "sac_seed0_300k_lift_progress"
BEST = FIX / "checkpoints/best_model.zip"
LAST = FIX / "checkpoints/last_model.zip"
...
def cmd(x):
    return subprocess.run(
        x, cwd=ROOT, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    ).stdout.strip()


def main():
    final = jload(BASE / "termination_ab_v2/final_summary.json")
    fc = jload(FIX / "final_model_comparison.json")
    cc = jload(CTRL / "final_model_comparison.json")
    rs = jload(BASE / "reward_ab_v1/final_summary.json")
    status = jload(FIX / "run_status.json")
    pstatus = jload(PARENT / "run_status.json")
    best = fc["best_model"]
    last = fc["last_model"]
    cb = cc["best_model"]
    cl = cc["last_model"]
    env = {
        ...
        "recommended_termination_mode": "fixed_horizon",
    }
    dump("final_environment_config.json", env)
    sac = {
        "algorithm": "SAC",
        "policy": "MlpPolicy",
    ...
    dump("final_sac_config.json", sac)
    obs = {
        "flat_dimension": 42,
        "dtype": "float32",
        "normalization": False,
        ...
        "keys_in_order": [
            {
                ...
                "dimension": 10,
                "components": [
                    {"name": "cube_pos", "indices": "0:3", "dimension": 3},
                    {"name": "cube_quat", "indices": "3:7", "dimension": 4},
                    {"name": "gripper_to_cube_pos", "indices": "7:10", "dimension": 3},
                ],
            },
            {
                ...
                "dimension": 32,
                "components": [
                    {"name": "joint_pos_cos", "indices": "10:17"},
                    {"name": "joint_pos_sin", "indices": "17:24"},
                    {"name": "joint_vel", "indices": "24:31"},
                    {"name": "eef_pos", "indices": "31:34"},
                    {"name": "eef_quat", "indices": "34:38"},
                    {"name": "gripper_qpos", "indices": "38:40"},
                    {"name": "gripper_qvel", "indices": "40:42"},
                ],
            },
        ],
        "observable": [
        ...
        "not_observable": [
            ...
            "hidden physics parameters",
            "future trajectory",
        ],
    }
    dump("observation_schema.json", obs)
    action = {
        "dimension": 7,
        "dtype": "float32",
        "space": "Box",
        ...
        "controller_outputs": {"translation_m": [-0.05, 0.05], "rotation_rad": [-0.5, 0.5]},
        "gripper_sign": "positive closes; negative opens",
        "sb3": "SAC squashed Gaussian is mapped to Box bounds; wrapper clips once",
        "audit": "No double scaling, gripper-sign inversion, or bounds mismatch found.",
    }
    dump("action_schema.json", action)
    success = {
        "source": "/workspace/collectenv/lib/python3.9/site-packages/robosuite/environments/manipulation/lift.py:_check_success",
        "exact_expression": "cube_height > table_height + 0.04",
        "table_height_m": 0.8,
        "threshold_m": 0.84,
        "grasp_required": False,
        "persistence_required": False,
        "velocity_constraint": False,
        "primary_metric": "ever_success",
        "secondary_metric": "final_success",
    }
    dump("success_definition.json", success)
    reward = {
        "mode": "control/original robosuite Lift shaped reward",
        "source": "/workspace/collectenv/lib/python3.9/site-packages/robosuite/environments/manipulation/lift.py:193-242",
        "unscaled": {
            "success": "2.25 if success; replaces shaped components",
            "reaching": "1-tanh(10*distance) before success",
            "grasp": "0.25 if grasped before success",
        },
        "scaling": "reward *= reward_scale / 2.25",
        "reward_scale": 1.0,
        "limitation": "No continuous lift-progress component before the 4 cm success threshold.",
        "excluded_treatment": {
            "name": "signed_lift_progress",
            "verdict": rs["LIFT_REWARD_EFFECT"],
            "best_success": rs["TREATMENT_SUCCESS_BEST"],
            "last_success": rs["TREATMENT_SUCCESS_LAST"],
        },
    }
    dump("reward_definition.json", reward)
    wrapper = {
        "robosuite_done": "ignored because ignore_done=True",
        "control_success": {"is_success": True, "terminated": True, "truncated": False},
        "fixed_success_before_horizon": {
            "is_success": True,
            "terminated": False,
            "truncated": False,
        },
        "horizon": {"terminated": False, "truncated": True},
        "sb3_timeout": "DummyVecEnv records TimeLimit.truncated; ReplayBuffer samples done*(1-timeouts)",
        "fixed_horizon_bootstrap": True,
        "success_termination_bootstrap": False,
    }
    dump("final_wrapper_semantics.json", wrapper)
    freeze = cmd([str(ROOT / ".venv_rl/bin/pip"), "freeze"])
    (OUT / "dependency_manifest.txt").write_text(
        "# Generated from .venv_rl/bin/pip freeze\n# robosuite resolves from /workspace/collectenv site-packages and is pinned below.\n"
        + freeze
        + "\n"
    )
    runtime = {
        "generated_at": "2026-08-30 UTC",
        "host_architecture": "aarch64",
        "kernel": platform.release(),
        "python": sys.version.splitlines()[0],
        "venv": str(ROOT / ".venv_rl"),
        "training_device": "cpu",
        "versions": {
            "robosuite": "1.2.0; pip provenance git cheng-chi/robosuite@277ab9588ad7a4f4b55cf75508b44aa67ec171f0",
            "robomimic": "NOT_INSTALLED_NOT_REQUIRED",
            "free-mujoco-py": "2.1.6",
            "MuJoCo_path_version": "/workspace/mujoco-2.1.1",
            "numpy": "1.23.3",
            "scipy": "1.9.1",
            "torch": "2.8.0+cpu",
        ...
        "environment_variables": {
            "PYGLFW_LIBRARY": "/workspace/collectenv/lib/libglfw.so.3",
            "MUJOCO_PY_MUJOCO_PATH": "/workspace/mujoco-2.1.1",
            "MUJOCO_GL": "osmesa",
            "LD_LIBRARY_PATH_prefix": [
                "/workspace/collectenv/lib",
                "/workspace/mujoco-2.1.1/bin",
                "/workspace/mujoco-2.1.1/lib",
            ],
        },
        "performance_note": "Low-dimensional MLP SAC is CPU-valid and primarily environment/MuJoCo-step limited.",
    }
    dump("environment_manifest.json", runtime)
    source_files = [
        "rl_baselines/envs.py",
        "rl_baselines/train_sac.py",
        "rl_baselines/train_sac_resume.py",
        "rl_baselines/evaluate.py",
        "rl_baselines/callbacks.py",
        "rl_baselines/progress.py",
        "rl_baselines/policy_adapter.py",
        "scripts/run_vanilla_sac.sh",
    ...
    prov = {
        "git": {
            ...
            "remote": cmd(["git", "remote", "-v"]) or "NONE",
            "status": cmd(["git", "status", "--short"]),
            "tracked_diff": cmd(["git", "diff", "--name-only"]) or "NONE",
        },
        "files": [{"path": x, "sha256": sha(ROOT / x)} for x in source_files],
        "external_local_source": {
            "robosuite_lift": "/workspace/collectenv/lib/python3.9/site-packages/robosuite/environments/manipulation/lift.py",
            "original_robotics_assets": "/home/robotics (read-only)",
        },
    }
    dump("source_provenance.json", prov)
    good = [
        {
            "designation": "RECOMMENDED_DOWNSTREAM_NOMINAL_POLICY",
            "path": str(BEST),
            "run": FIX.name,
            "effective_step": 280000,
            "sha256": sha(BEST),
            "termination": "fixed_horizon",
            "reward": "original shaped Lift",
            "seed": 0,
            "evaluation": {
                "ever_success": best["success_rate"],
                "final_success": best["final_success_rate"],
                "lift": best["lift_rate"],
                "p_lift_given_grasp": best["p_lift_given_grasp"],
            },
            "use": "deterministic nominal closed-loop policy; seed-0 only",
        },
        {
            "designation": "STABLE_LAST_REFERENCE",
            "path": str(LAST),
            "run": FIX.name,
            "effective_step": 300000,
            "sha256": sha(LAST),
            "termination": "fixed_horizon",
            "reward": "original shaped Lift",
            "seed": 0,
            "evaluation": {
                "ever_success": last["success_rate"],
                "final_success": last["final_success_rate"],
                "lift": last["lift_rate"],
                "p_lift_given_grasp": last["p_lift_given_grasp"],
            },
            "use": "late-training stability reference",
        },
    ]
    dump("known_good_checkpoints.json", good)
    nongood = [
        {
            "role": "diagnostic parent; not nominal",
            "path": str(BASE / "sac_seed0_100k_diag/checkpoints/last_model.zip"),
            "reason": "success-termination, 5% success",
        },
        {
            "role": "control grasp-plateau",
            "path": str(CTRL / "checkpoints/best_model.zip"),
            "reason": "0% success, 2% lift at final matched evaluation",
        },
        {
            "role": "control late plateau",
            "path": str(CTRL / "checkpoints/last_model.zip"),
            "reason": "0% success/lift",
        },
        {
            "role": "reward-treatment diagnostic",
            "path": str(REWARD / "checkpoints/best_model.zip"),
            "reason": "signed lift-progress is not the final reward and regressed",
        },
    ]
    for x in nongood:
        x["sha256"] = sha(x["path"]) if Path(x["path"]).exists() else "UNKNOWN"
    dump("known_bad_or_nonfinal_checkpoints.json", nongood)
    history = [
        ["phase", "run", "steps", "semantics", "reward", "purpose", "result", "verdict"],
        [
            "A",
            "sac_20k_sanity / sac_20k_sanity_tqdm",
            "20k",
            "success termination",
            "original",
            "pipeline sanity",
            "numerical updates; task not learned",
            "PASS_NUMERIC_ONLY",
        ],
        [
            "B",
            ...
            "original",
            "initial control",
            "best 10%, last 5%; reach/grasp learned",
            "PARTIAL_LEARNING",
        ],
        [
            "C",
            "sac_seed0_300k_from_last",
            "100k->300k continuation",
            "success termination",
            "original",
            "budget test",
            "best/last success 0%; grasp 100%/95%",
            "PLATEAU_AT_GRASP",
        ],
        [
            "D",
            "sac_seed0_300k_lift_progress",
            "300k fresh",
            "success termination",
            "signed lift progress",
            "reward A/B",
            "best 5%, last 1%; unstable",
            "NOT_SUPPORTED_WITH_THIS_SHAPING",
        ],
        [
            "E1",
            "sac_seed0_100k_fixed_horizon",
            "100k fresh",
            "fixed horizon",
            "original",
            "termination A/B parent",
            "best 7%, last 6%",
            "VALID_FIXED_PARENT",
        ],
        [
    ...
    with (OUT / "experiment_history.csv").open("w", newline="") as f:
    ...
    (OUT / "command_reference.md").write_text(commands)
    ...
    (OUT / "artifact_index.md").write_text(artifacts)
    ...
    (OUT / "reproduction_checklist.md").write_text(checklist)
    ...
    (OUT / "troubleshooting.md").write_text(trouble)
    summary = {
        "BASELINE_NAME": "FaRL vanilla SAC fixed-horizon seed0 baseline",
        "TASK": "robosuite Lift",
        "ROBOT": "Panda",
        "CONTROLLER": "OSC_POSE",
        "CONTROL_FREQ": 20,
        "HORIZON": 200,
        "OBSERVATION_TYPE": "low-dimensional state; no RGB",
        "OBS_DIM": 42,
        "ACTION_DIM": 7,
        "REWARD_MODE": "original robosuite Lift dense shaped reward",
        "REWARD_SCALE": 1.0,
        "SUCCESS_CRITERION": "cube_height > table_height + 0.04 m",
